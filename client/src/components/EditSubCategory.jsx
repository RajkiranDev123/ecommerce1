...
const EditSubCategory = ({ close, data, fetchData }) => {
  ...
  const [subCategoryData, setSubCategoryData] = useState({
   _id:data?._id, name: data?.name, image: data?.image, category: data?.category || []
  })
    ...
    <section className='fixed top-0 bottom-0 flex justify-center items-center left-0 right-0 bg-neutral-500 bg-opacity-50'>
      <div className='bg-white max-w-4xl w-full p-2 rounded flex-col'>
        ...
        <form className='grid' onSubmit={handleOnSubmit}>
          <p className='font-semibold'>Edit Sub Category</p>
          ...
          <button className={`p-1 ${subCategoryData.image && subCategoryData.name && subCategoryData?.category[0] ? "cursor-pointer bg-green-500" : "bg-slate-400 cursor-not-allowed"} text-white rounded mt-1`}>Update</button>
